...
In the schema.xml file you'll find my fields within which i have a field named "pSearchField" which is only supposed to work with proximity it has no synonym filter or stemmer in it . But still I am unable to get it working right although in the analysis tab the stopword filter factory is stopping the words as its suppose to but in query time the proximity search is not stopping the stopwords .
